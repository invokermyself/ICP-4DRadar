...
MatrixXd circshift( MatrixXd &_mat, int _num_shift )
{
    ...
    //     }
    //     MatrixXd res_mat = shifted_mat.topRightCorner(_mat.rows(), _mat.cols() - _num_shift);
    //         return res_mat;
    // }
    ...
    //         shifted_mat.col(new_location) = _mat.col(col_idx);
    //     }
    //     MatrixXd res_mat = shifted_mat.topLeftCorner(_mat.rows(), _num_shift);
    //         return res_mat;
    // }
...
double SCManager::distDirectSC ( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    if(num_eff_cols > 4)
    {
        sc_sim = sum_sector_similarity / num_eff_cols;
    }
    else{
        sc_sim = 0;
    }
...
int SCManager::fastAlignUsingVkey( MatrixXd & _vkey1, MatrixXd & _vkey2)
{
    ...
    for ( int shift_idx = 0; shift_idx < _vkey1.cols(); shift_idx++ )
    {
        ...
        // MatrixXd vkey1_shifted = circshift(_vkey1, -shift_idx);
...
std::pair<double, int> SCManager::distanceBtnScanContext( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    for ( int num_shift: shift_idx_search_space )
    {
        ...
        // MatrixXd sc1_shifted = circshift(_sc1, -num_shift);
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        ...
        sctor_idx = std::max( std::min( PC_NUM_SECTOR, int(ceil( (azim_angle / 120.0) * PC_NUM_SECTOR )) ), 1 );
